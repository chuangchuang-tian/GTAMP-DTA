...
METAL_SET = ["LI", "NA", "K", "RB", "CS", "MG", "TL", "CU", "AG", "BE", "NI", "PT", "ZN", "CO", "PD", "AG", "CR", "FE", "V",
         "MN", "HG", 'GA',
...
RES_SET = {"A": 1, "C": 2, "B": 3, "E": 4, "D": 5, "G": 6,
               "F": 7, "I": 8, "H": 9, "K": 10, "M": 11, "L": 12,
               "O": 13, "N": 14, "Q": 15, "P": 16, "S": 17, "R": 18,
               "U": 19, "T": 20, "W": 21, "V": 22, "Y": 23, "X": 24, "Z": 25}


def sequence_embedding(line, set, max_length=1000):
    X = np.zeros(max_length, np.int64())
...
def prot_to_graph(id, prot_pdb, thershold=10.0):
    ...
    edgeids, distm = obatin_edge(u, thershold)
...
if __name__ == '__main__':
    ...
    file_path_protein = '/2111041015/GTAMP-DTA/data/' + dataset + '/DTA/' + dataset + '_protein_mapping.csv'
    ...
    for no, data in enumerate(protein_id_unique):
         ...
         protein_graph = prot_to_graph(data, pdb_pdb, thershold=10.0)
         # protein_embedding = sequence_embedding(data, RES_SET, protein_max_len)
         proteins_g.append(protein_graph)
         dgl.save_graphs(dir_output + '/pocket_graph/' + data + '.bin', list(proteins_g))
    ...
    #         'data/' + dataset + '/processed' + '/pocket_graph1/' + str(data) + '.bin')
    #     feats = protein_graph[0].ndata['feats'][:, 41:]
    #     np.save(dir_output + '/ESM_embedding_pocket/' + str(data) + '.npy', feats)
    #     end = timeit.default_timer()
    #     time = end - start
    #     print(round(time, 2))
    ...
    for no, id in enumerate(protein_id_train):
        print('/'.join(map(str, [no + 1, N])))
        protein_embedding_train = np.load('/2111041015/GTAMP-DTA/data/' + dataset + '/processed' + '/ESM_embedding/' + str(id) + '.npy', allow_pickle=True)
    ...
    for no, id in enumerate(protein_id_test):
        print('/'.join(map(str, [no + 1, N])))
        protein_embedding_test = np.load('/2111041015/GTAMP-DTA/data/' + dataset + '/processed' + '/ESM_embedding/' + str(id) + '.npy', allow_pickle=True)
    ...
    for no, id in enumerate(protein_id_train):
        print('/'.join(map(str, [no + 1, N])))
        protein_graph_train, _ = load_graphs('/2111041015/GTAMP-DTA/data/' + dataset + '/processed' + '/pocket_graph/' + str(id) + '.bin')
    ...
    for no, id in enumerate(protein_id_test):
        print('/'.join(map(str, [no + 1, N])))
        protein_graph_test, _ = load_graphs('/2111041015/GTAMP-DTA/data/' + dataset + '/processed' + '/pocket_graph/' + str(id) + '.bin')
